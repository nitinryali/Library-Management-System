...
def sign_in(request):
    ...
    if(len(x1)==0):
        ...
        if len(my_string)<=10:
                 my_string=my_string 
        elif  len(my_string)==40:
               my_string=my_string[10:20]
        else:
...
def option(request):
    return redirect("https://remix.ethereum.org/#optimize=false&runs=200&evmVersion=null&version=soljson-v0.8.13+commit.abaa5c0e.js")
def home(request):
    return render(request,'index1.html')
def home1(request):
    return render(request,'index.html')
